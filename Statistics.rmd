...
```{r}
data <- read.csv(text=getURL("https://raw.githubusercontent.com/cemborski/DynamicTransgenerationalPlasticity_SupplementaryMaterials/master/data.csv"), header=T)
...
write.csv(SuppTable1Sig, file = "Supplementary Table 1.Sex Specific Comparisons.csv")
...
#Create Dataframe of Results (See Fit.results.df)
lastThree <- function(x) substr(x, nchar(x)-2, nchar(x))
Fit.M.results.df <- rbind(tidy(Fit.0M.Mod) %>% mutate(model = "F0"),
                 tidy(Fit.1M.Mod) %>% mutate(model = "F1"),
                 tidy(Fit.2M.Mod) %>% mutate(model = "F2"),
                 tidy(Fit.3M.Mod) %>% mutate(model = "F3"),
                 tidy(Fit.4M.Mod) %>% mutate(model = "F4"),
                 tidy(Fit.5M.Mod) %>% mutate(model = "F5")) %>%
  filter(grepl("Treatment", term)) %>% 
  mutate(diet = lastThree(term)) %>% 
  mutate(sex = ifelse(grepl("Sexfemale", term), "female", "male")) %>% 
  filter(!grepl("female", sex))
...
write.csv(AllSig, file = "Supplementary Table 2.Treatment vs. Control Contrast FDR corrected (AllSig).csv")
...
SW.Norm <- SW.Norm[with(SW.Norm, order(Metabolite)),]
Glucose <- SW.Norm[c(1:1577),c(1:3,8)]
colnames(Glucose)[4] <- "Glucose"
Trehalose <- SW.Norm[c(1578:3154),c(1:3,8)]
colnames(Trehalose)[4] <- "Trehalose"
Glycogen <- SW.Norm[c(3155:4731),c(1:3,8)]
colnames(Glycogen)[4] <- "Glycogen"
...
F.T.N.p <- ggplot(subset(Trehalose, Sex=="female" & Treatment=="NSD")) + geom_boxplot(aes(x=Generation, y=Trehalose), fill="#cccccc") + ggtitle("Trehalose") + ylab(" ") + xlab(" ") + theme_bw() + geom_hline(aes(yintercept=1.0), color="black", linetype="dashed")  + scale_y_continuous(limits=c(0, 2.5)) + theme(legend.position="none", axis.text=element_text(size=25), axis.title.y = element_text(size = rel(2.5), angle = 90), plot.title = element_text(size = rel(3), hjust = 0.5)) + 
  ...
  annotate("text", 5, y = 1.9, label = "***", cex=12) + 
  annotate("text", 6, y = 0.08, label = "", cex=12) +
  annotate("text", 6, y = 0.08, label = "A", cex=16) 
F.T.N.p
...
F.Y.N.p <- ggplot(subset(Glycogen, Sex=="female" & Treatment=="NSD")) + geom_boxplot(aes(x=Generation, y=Glycogen), fill="#cccccc") + ggtitle("Glycogen") + ylab(" ") + xlab(" ") + theme_bw() + geom_hline(aes(yintercept=1.0), color="black", linetype="dashed")  + scale_y_continuous(limits=c(0, 2.5)) + theme(legend.position="none", axis.text=element_text(size=25), axis.title.y = element_text(size = rel(2.5), angle = 90), plot.title = element_text(size = rel(3), hjust = 0.5)) + 
  ...
  annotate("text", 5, y = 2.165, label = "***", cex=12) + 
  annotate("text", 6, y = 0.08, label = "", cex=12) +
...
F.Y.L.p <- ggplot(subset(Glycogen, Sex=="female" & Treatment=="LSD")) + geom_boxplot(aes(x=Generation, y=Glycogen), fill="#cccccc") + ggtitle("") + ylab(" ") + xlab(" ") + theme_bw() + geom_hline(aes(yintercept=1.0), color="black", linetype="dashed")  + scale_y_continuous(limits=c(0, 2.5)) + theme(legend.position="none", axis.text=element_text(size=25), axis.title.y = element_text(size = rel(2.5), angle = 90), plot.title = element_text(size = rel(3), hjust = 0.5)) + 
  annotate("text", 1, y = 1, label = "***", cex=12) + 
  annotate("text", 2, y = 1.75, label = "", cex=12) + 
  annotate("text", 3, y = 1.97, label = "*", cex=12) + 
  annotate("text", 4, y = 1.75, label = "*", cex=12) + 
...
F.Y.H.p <- ggplot(subset(Glycogen, Sex=="female" & Treatment=="HSD")) + geom_boxplot(aes(x=Generation, y=Glycogen), fill="#cccccc") + ggtitle("") + ylab(" ") + xlab(" ") + theme_bw() + geom_hline(aes(yintercept=1.0), color="black", linetype="dashed")  + scale_y_continuous(limits=c(0, 2.5)) + theme(legend.position="none", axis.text=element_text(size=25), axis.title.y = element_text(size = rel(2.5), angle = 90), plot.title = element_text(size = rel(3), hjust = 0.5)) + 
  annotate("text", 1, y = 1.755, label = "***", cex=12) + 
  annotate("text", 2, y = 2.18, label = "**", cex=12) + 
  annotate("text", 3, y = 0.55, label = "", cex=12) + 
  annotate("text", 4, y = 1.75, label = "", cex=12) + 
  annotate("text", 5, y = 1.6, label = "", cex=12) + 
  annotate("text", 6, y = 1.5, label = "**", cex=12)  +
...
CorrData <- CorrData[-c(1:164),]

#Separate Metabolites/Fitness data points
Sugar <- CorrData[,c(1:3,5:8)] #Sugar Data only
...
SW.Norm <- SW.Norm[with(SW.Norm, order(Metabolite)),]
Glucose <- SW.Norm[c(1:1489),c(1:3,8)]
Trehalose <- SW.Norm[c(1490:2978),c(1:3,8)]
Glycogen <- SW.Norm[c(2979:4467),c(1:3,8)]
...
NvL_H <- ggplot() + geom_point(data=CombinedMetabolites.p, aes(x=NSD, y=LSD, color=NvL, shape=Metabolite), size=3) + geom_smooth(data=CombinedMetabolites.p, aes(x=NSD, y=LSD), method = "lm", se = FALSE, colour="red") + geom_point(data=CombinedMetabolites.p, aes(x=NSD, y=HSD, color=NvH, shape=Metabolite), size=3) + geom_smooth(data=CombinedMetabolites.p, aes(x=NSD, y=HSD), method = "lm", se = FALSE, colour="#0000FF") + theme_bw() +  scale_color_manual(values=c("#0000FF", "red")) + scale_shape_manual(values=c(15:18,0:3)) + ggtitle("Relationships Between Treatments") + ylab("LSD or HSD") +  
  theme(axis.text=element_text(size=16),
        ...
        annotate("text", 0.85, y = 1.5, 
            label = "NSD v HSD: \nR=0.1490687 [95% CI: -0.1703511-0.4401396] \n p=0.3586", cex=5, color="#0000FF") + annotate("text", 0.85, y = 1.7, 
            label = "NSD v LSD: \nR=0.6075067 [95% CI: 0.365088-0.7727733] \n p=3.22e-05", cex=5, color="red") 

ggsave("Relationships Between Treatments Scatterplot.png", NvL_H, width = 10, height = 8, units = "in")



# ##SEPARATE METABOLITES CORR.TEST TO VERIFY COMBINING ALL WITHIN ONE CORRELATION:
# #Separate Treatments
# GlucoseCorr <- GlucoseCorr[with(GlucoseCorr, order(Treatment)),]
# NG <- GlucoseCorr[c(11:20),]
# LG <- GlucoseCorr[c(21:30),]
# HG <- GlucoseCorr[c(31:40),]
# cor.test(NG$Glucose, LG$Glucose)
# cor.test(NG$Glucose, HG$Glucose)
# 
# TrehaloseCorr <- TrehaloseCorr[with(TrehaloseCorr, order(Treatment)),]
# NT <- TrehaloseCorr[c(11:20),]
# LT <- TrehaloseCorr[c(21:30),]
# HT <- TrehaloseCorr[c(31:40),]
# cor.test(NT$Trehalose, LT$Trehalose)
# cor.test(NT$Trehalose, HT$Trehalose)
# 
# GlycogenCorr <- GlycogenCorr[with(GlycogenCorr, order(Treatment)),]
# NY <- GlycogenCorr[c(11:20),]
# LY <- GlycogenCorr[c(21:30),]
# HY <- GlycogenCorr[c(31:40),]
# cor.test(NY$Glycogen, LY$Glycogen)
# cor.test(NY$Glycogen, HY$Glycogen)
# 
# TAGCorr <- TAGCorr[with(TAGCorr, order(Treatment)),]
# NTa <- TAGCorr[c(11:20),]
# LTa <- TAGCorr[c(21:30),]
# HTa <- TAGCorr[c(31:40),]
# cor.test(NTa$TAG, LTa$TAG)
# cor.test(NTa$TAG, HTa$TAG)
# 
##Or further By sex and metabolite
...
Pilot vs Main Study
```{r}
dataPM <- data <- read.csv(text=getURL("https://raw.githubusercontent.com/cemborski/DynamicTransgenerationalPlasticity_SupplementaryMaterials/master/PilotvMain_Data.csv"), header=T)

dataPM.m <- melt(dataPM, id.vars=c("Study", "Generation", "Sex", "Treatment", "Sugar.Weight"), measure.vars=c("Glucose", "Trehalose", "Glycogen"))
  

#Create dataframe that normalizes each generation to controls in order to account for standard degradation (and minor variation due to hand mixing of standards) between generations.
...
dataPM.Norm <- data.frame(Study=character(), Generation=character(), Sex=character(), Treatment=character(), variable=character(), value=numeric())
for(i in unique(dataPM.SW$Study))
  for(j in unique(dataPM.SW$Generation))
    for(k in unique(dataPM.SW$Sex))
      for(l in unique(dataPM.SW$Treatment))
        for(m in unique(dataPM.SW$variable)){
        temp <- subset(dataPM.SW, Study == i & Generation == j & Sex == k & Treatment == l & variable == m)
        temp$PercentMetabWt <- (subset(dataPM.SW, Study == i & Generation == j & Sex == k & Treatment == l & variable == m)$MetabWt)/mean(subset(dataPM.SW, Study == i & Generation == j & Sex == k & Treatment == "CD" & variable == m)$MetabWt)
dataPM.Norm <-  rbind(dataPM.Norm, temp)
      }



dataPM.Norm.mean <-aggregate(dataPM.Norm$PercentMetabWt, by = list(dataPM.Norm$Study, dataPM.Norm$Generation, dataPM.Norm$Treatment, dataPM.Norm$Sex, dataPM.Norm$variable),  mean)

colnames(dataPM.Norm.mean)[1:5] <- c("Study", "Generation", "Treatment", "Sex", "Metabolite")

#Remove Controls from Data (all = to 1, given normalization)
dataPM.Norm.mean <- dataPM.Norm.mean[with(dataPM.Norm.mean, order(Treatment)),]
dataPM.Norm.mean <- dataPM.Norm.mean[-c(1:72),]

#Arrange Dataframe to plot main vs. pilot
dataPM.Norm.mean <- dataPM.Norm.mean[with(dataPM.Norm.mean, order(Study)),]
dataP.Norm.mean <- dataPM.Norm.mean[-c(1:108),]
dataM.Norm.mean <- dataPM.Norm.mean[-c(109:216),]
PlotDataPM <- merge(dataP.Norm.mean, dataM.Norm.mean, by=c("Generation", "Treatment", "Sex", "Metabolite"))
...
Main_v_Pilot.p <- ggplot(PlotDataPM,aes(x=Pilot, y=Main, color=Generation, shape=SexTreatment)) + 
  ...
  theme(axis.text=element_text(size=16),
        ...
        annotate("text", 0.85, y = 1.75, 
            label = "R=0.501 [95% CI: 0.345-0.630] \n p=3.334e-08", cex=5) + 
        scale_color_manual(values=c( "#CC33FF","#0000FF","#33FFFF", "#00FF66","#FFFF00","#FF3300")) + scale_shape_manual(values=c(15:17,0:2))
